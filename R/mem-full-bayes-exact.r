...
mem_full_bayes_exact <- function(responses,
  ...
  log.Marg <-
    foreach(mod_i = isplitRows(Mod.I, chunks = num_workers()),
            .combine = c) %dopar% {
              apply(mod_i,
                    MARGIN = 1,
                    FUN = logMarg.Dens,
  ...
  PRIOR <-
    apply(Mod.I, MARGIN = 1, FUN = mem.Prior, mod.mat, pr.Inclus)
...
sample_posterior.full_bayes <-
  function(model, num_samples = 10000) {
    ret <- replicate(
      num_samples,
      samp.Post(model$responses, model$size,
