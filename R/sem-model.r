...
#' @importFrom rjags jags.model update
#' @export
sem <- function(responses, basket_size, p0 = 0.25, mu = 0, M = 0, inv_sig = 1, 
  num_chains = 3, quiet = TRUE, num_iterations = 3000, 
  ...
  if (length(mu) == 1) {
    mu <- rep(mu, length(basket_size))
  }
  inits <- list(mu = mu, M = M, inv.sig2 = inv_sig)
  data <- list(J = length(responses), N = basket_size, Y = responses)
  ...
  ret <- list(jags_model = jags_model, responses = responses, 
              basket_size = basket_size, p0 = p0, mu = mu, M = M, 
