...
########################################
N <- rep(25,8); prob=rep(0.15,8)                    ## Generate responses for N baskets with prob vector prob
Data <- list( X=rbinom(N, N, prob), N=N )             ## Vectors of Observed number of Responses (X) and Patients (N)
...
## Emp Bayes optimal MMLE MEM ##
 ...
 marg.M <- MEM_marginalLoop(Data$X, Data$N, rep(0.5,length(Data$X)), rep(0.5,length(Data$X)), INITIAL=diag(1,length(Data$X))); marg.M$maximizer
...
## Full Bayesian with Reference Prior ##
 ...
 FullBayes.anal <- MEM_FullBayes(Data$X, Data$N, rep(0.5,length(Data$X)), rep(0.5,length(Data$X)), pr.Inclus, 0.05)
