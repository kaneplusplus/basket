...
#    no_cores <- detectCores() - 1;  cl <- makeCluster(no_cores); registerDoParallel(cl)
     pweights <- foreach( j = 1:length(xvec), .combine = list, .multicombine = TRUE)  %dopar% post.Weights(j, length(xvec), Mod.I, mod.mat, pr.Inclus, log.Marg, PRIOR)
    #stopCluster(cl)
  ...
  ### SEM ###
   library(rjags)
    ...
    m <- jags.model("../Utils/Berry-ig_model.txt", data, inits, n.chains=3, quiet=TRUE); update(m, 10000, progress.bar="none")
