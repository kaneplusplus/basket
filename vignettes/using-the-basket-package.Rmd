...
exact_res <- mem_exact(responses = vemu_wide1$responders, 
                     size = vemu_wide1$evaluable,
                     name = vemu_wide1$baskets, p0=rep(0.15, length(baskets)))
```
The output "exact_res" is a list including three components:  call, basket, clusterwise.
...
The "basketwise" includes all results of the basket analyses.

```{r}
print(exact_res$basket$PEP)
print(exact_res$basket$MAP)
print(exact_res$basket$post.prob)
print(exact_res$basket$HPD)
print(exact_res$basket$ESS)
print(exact_res$basket$mean_est)
print(exact_res$basket$median_est)
```
The "clusterwise" includes all results of the clusterwise analyses.
```{r}
print(exact_res$cluster$cluster)
print(exact_res$cluster$post.prob)
print(exact_res$cluster$HPD)
print(exact_res$cluster$ESS)
print(exact_res$cluster$mean_est)
print(exact_res$cluster$median_est)
...
MHResult1 <- mem_mcmc(responses = vemu_wide2$responders, 
...
print(MHResult1$basket$PEP)
print(MHResult1$basket$MAP)
print(MHResult1$basket$post.prob)
print(MHResult1$basket$HPD)
print(MHResult1$basket$ESS)
print(MHResult1$basket$mean_est)
print(MHResult1$basket$median_est)


print(MHResult1$cluster$cluster)
print(MHResult1$cluster$post.prob)
print(MHResult1$cluster$HPD)
print(MHResult1$cluster$ESS)
print(MHResult1$cluster$mean_est)
print(MHResult1$cluster$median_est)
```
...
summary(exact_res$basket)
summary(exact_res$cluster)
```
...
plot_mem(exact_res$basket, c("PRIOR", "MAP", "PEP"), 
...
plot_mem(exact_res$basket, c("MAP", "PEP"),
