...
As a case study, consider the recent [vemurafenib study that was performed for multiple nonmelanoma cancers with BRAF V600 Mutations by Hynman et al.](https://www.nejm.org/doi/full/10.1056/NEJMoa1502309) This was a basket study to assess the efficacy of vemurafinib for Non-Small Cell Lung Cancer (NSCLC), ColoReCtal (CRC) treated with vemurafenib only, CRC treated with both vemurafenib and cetuximab, cholangiocarcinoma (Bile Duct), Erdheim–Chester disease or Langerhans’-cell histiocytosis (ECD/LCH), and anaplastic thyroid cancer (ATC). The results of the trials are summarized in the following table.
...
Along with the densities, the package can show the exchangeability between the individual arms via, what we are terming, an __exchangeogram__. This is similar to a correlogram but, rather than showing correlations among varibles, an exchangeogram shows the exchangeability between arms. Exchangeograms for MEM analyses can be created using the `plot_mem()` function. By default the prior exchangeability, MAP exchangeability, and posterior exchangeability are shown in a single plot, and is controlled by the default parameter `type = c("prior", "map", "pep")`. Along with the visualization, the MAP and PEP matrices can retrieved with the `basket_map()` and `basket_pep()` function for baskets and `cluster_map()` and `cluster_pep()` function for clusters.
