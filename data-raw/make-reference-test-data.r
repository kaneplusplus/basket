...
pars = list(UB = 1, LB = 0)
eb_reference <- list(marg_m = marg_m, maximizer = marg_m$maximizer,
...
eb_reference_ub <- list(marg_m = marg_m, maximizer = marg_m$maximizer,
                        cdf = MEM.cdf(Data, pars, p0, marg_m),
                        ess = post.ESS(Data, pars, marg_m),
                        hpd = t(post.HPD(Data, pars, marg_m, 0.05)))
...
fb_reference <- MEM_FullBayes(Data$X, Data$N, rep(0.5, length(Data$X)),
